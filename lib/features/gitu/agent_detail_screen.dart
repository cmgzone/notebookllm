...
class AgentDetailScreen extends ConsumerWidget {
        ...
        data: (agent) {
          return ListView(
            ...
            children: [
              ...
              _buildSectionTitle(context, 'Task'),
              _buildCard(
                  Text(agent.task, style: const TextStyle(fontSize: 16))),
              const SizedBox(height: 24),
              ...
              if (agent.status == 'completed' && agent.result != null) ...[
                ...
                _buildCard(
                    MarkdownBody(data: agent.result?['output'] ?? 'No output')),
              ] else if (agent.memory.isNotEmpty) ...[
                _buildSectionTitle(context, 'Thought Process'),
  ...
  Widget _buildHeader(GituAgent agent) {
    return Column(
      ...
      children: [
        Text('Agent ID: ${agent.id.substring(0, 8)}',
            style: const TextStyle(color: Colors.grey)),
        Text('Created: ${agent.createdAt.toLocal()}',
            style: const TextStyle(color: Colors.grey)),
      ],
    );
  }
