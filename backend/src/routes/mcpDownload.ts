...
router.get('/package.tgz', async (req: Request, res: Response) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="notebookllm-mcp.tgz"');
...
# NotebookLLM MCP Server Installer
...
echo ""
echo "✅ NotebookLLM MCP Server installed to $MCP_DIR"
echo ""
...
router.get('/install.ps1', async (req: Request, res: Response) => {
  ...
  const script = `# NotebookLLM MCP Server Installer for Windows
...
# Download the server
Write-Host "⬇️  Downloading MCP server..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "✅ NotebookLLM MCP Server installed to $MCP_DIR" -ForegroundColor Green
Write-Host ""
...
router.get('/config', async (req: Request, res: Response) => {
  ...
  const mcpDir = process.platform === 'win32' 
    ? '%USERPROFILE%\\.notebookllm-mcp'
    : '$HOME/.notebookllm-mcp';
